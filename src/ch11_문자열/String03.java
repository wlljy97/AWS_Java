	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < addressArray.length; i++) {
			if(addressArray[i].contains("부산시")) {
				...
				tempArray[tempArray.length - 1] = addressArray[i];
				// (↑) tempArray[] 배열안의 값에 tempArray.length - 1 를 넣어도 되고 addressArray2.length 를 넣어도 된다.
				...
				// 마지막 배열 공간에 새로운 값을 대입한다.
				tempArray[tempArray.length - 1] = addressArray[i];
				// (↑) tempArray[] 배열안의 값에 tempArray.length - 1 를 넣어도 되고 addressArray3.length 를 넣어도 된다.
